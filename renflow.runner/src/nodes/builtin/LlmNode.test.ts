...
test('LlmNode agent mode forwards normalized params to runtime factory', async () => {
    ...
    globalThis.fetch = (async (_input: any, init?: any) => {
        ...
        if (body.method === 'initialize') {
            return new Response(JSON.stringify({ result: { protocolVersion: '2024-11-05', capabilities: {} } }), {
                status: 200,
    ...
    try {
        ...
        assert.equal(calls[0].tools[0].name, 'get_time')
        assert.equal(result.output.sessionInfo.mcpServers[0].serverLabel, 'docs')
        assert.equal(result.output.sessionInfo.mcpServers[0].requireApproval, 'always')
    } finally {
